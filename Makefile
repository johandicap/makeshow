...
ruff_check:
	...
	@ruff check --config ./pyproject.toml $(PYTHON_FILES_AND_FOLDERS)

ruff_fix:
	$(call header,"[make ruff_fix]")
	@ruff check --config ./pyproject.toml --fix --exit-non-zero-on-fix $(PYTHON_FILES_AND_FOLDERS)
...
.PHONY: isort_check isort_fix black_check black_fix ruff_check ruff_fix mypy fix ci_no_test ci
